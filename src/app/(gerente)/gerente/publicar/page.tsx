...
export default function GerentePublicarPage() {
  ...
  // Tipos com template disponível — form_templates + system_config.
  // system_config.tmpl_* de formType="lamina" é mapeado pra "quatro_destinos"
  ...
  // Se o cliente não tem template em um formato, o pill não aparece.
  ...
  // Tipos com template (form_templates + system_config). "lamina" vira "quatro_destinos"
  // (mesmo form de Cards). Só aparecem tipos suportados pela UI do form.
  ...
  // Formatos disponíveis pra um tipo específico (usado no seletor abaixo do preview)
    ...
    try {
      ...
      const rows: TemplateRow[] = (tplData ?? []).map((r) => {
        ...
        return {
          ...
          width: row.width || defW,
          height: row.height || defH,
          schema: {
      ...
      // system_config.tmpl_* — templates legados (base ou do licensee) que ainda não migraram pra form_templates.
      ...
      try {
        ...
        for (const r of (sc ?? []) as { key: string; value: string }[]) {
          try {
            ...
            // Mantém base ou do licensee atual (ignora templates de outros clientes).
            if (!isBase) {
              if (!lid) continue;
              const lidTrim = String(lid).trim();
              if (lidTrim !== String(p.licensee_id).trim()) continue;
            }
