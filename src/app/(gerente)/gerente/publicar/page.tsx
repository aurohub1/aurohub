  ...
  async function handleThumbUpload(key: string, file: File) {
    ...
    try {
      ...
      const res = await fetch("/api/upload-thumb", {
        ...
        body: JSON.stringify({ dataUrl, folder: "aurohubv2/thumbs" }),
      });
      ...
      // Update local state
      setTemplates((prev) => prev.map((t) => t.key === key ? { ...t, thumbnail: url } : t));
    } catch (err) {
      console.error("[Gerente] Thumb upload:", err);
    } finally {
              ...
              >
                ...
                <div style={{ padding: "10px 12px" }}>
                  ...
                  <div style={{ marginTop: 6 }}>
                    <label style={{ display: "inline-flex", alignItems: "center", gap: 4, fontSize: 11, color: "var(--txt3)", cursor: "pointer" }}>
                      <input
                        type="file"
                        accept="image/*"
                        style={{ display: "none" }}
                        disabled={thumbUploading === t.key}
                        onChange={(e) => { const f = e.target.files?.[0]; if (f) handleThumbUpload(t.key, f); e.currentTarget.value = ""; }}
                      />
                      📷 {thumbUploading === t.key ? "Enviando…" : "Thumb"}
                    </label>
                  </div>
                </div>
                <button
