...
function RenderEl({ el, values, formType }: { el: EditorElement; values: Record<string, string>; formType?: string }) {
  ...
  if (el.type === "text") {
    ...
    // Se texto está vazio, não renderiza
    if (!txt) return null;
    ...
    if (el.priceDisplay && el.bindParam && ["valorparcela","valorint"].includes(el.bindParam)) {
      ...
      const fillProps = getFillProps(el.fill, el.width, textHeight);
    ...
    const textHeight = el.linhas ? Math.ceil(fSize * (el.lineHeight ?? 1.2) * el.linhas) : Math.ceil(fSize * (el.lineHeight ?? 1.2));
    const textFillProps = getFillProps(el.fill, el.width, textHeight);
    return (
      <KText
        x={el.x} y={el.y}
        width={el.width}
        height={el.linhas ? textHeight : undefined}
        wrap="word"
        ellipsis={!!el.linhas}
        rotation={el.rotation ?? 0}
        text={txt}
...
export default function PreviewStage({ schema, width, height, values, maxDisplay = 420, onReady }: Props) {
  ...
  const resolvedElements = useMemo(() => {
    ...
    // Coleta todos os IDs que são alvo de algum seguidor (smartTrack/textAnchor/autoHeightRef/smartResize)
    ...
    // Propaga cascata de smart-links para cada alvo identificado
