...
export default function GerenteChatPage() {
  ...
  const loadRooms = useCallback(async (licenseeId: string, userId: string) => {
    ...
    // Lojas que este gerente gerencia
    const { data: userStores } = await supabase
      .from("user_stores")
      .select("store_id")
      .eq("user_id", userId);
    const storeIds = ((userStores ?? []) as { store_id: string }[]).map(s => s.store_id).filter(Boolean);

    // Salas das lojas do gerente + sala da franquia (store_id null)
    let query = supabase
      .from("chat_rooms")
    ...
    const { data: rawRooms } = await query.order("created_at", { ascending: true });
    ...
    const enriched: Room[] = (rawRooms as { id: string; name: string; store_id: string | null; created_at: string }[]).map(r => {
      ...
      return { id: r.id, name: r.name, store_id: r.store_id ?? null, created_at: r.created_at, last_msg: lastMsg, unread };
    });
  ...
  useEffect(() => {
    ...
    const ch = supabase
      .channel(`gerente-chat-room-${activeRoomId}`)
      .on(
        "postgres_changes",
  ...
  const sendMessage = useCallback(async () => {
    ...
    try {
      await supabase.from("chat_messages").insert({
        ...
        user_id: profile.id,
        sender_name: profile.name ?? "Gerente",
        message: text,
      });
    } finally {
      setSending(false);
