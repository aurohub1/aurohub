    ...
    (async () => {
      try {
        ...
        if (data?.value) {
          ...
          if (parsed.elements) setSchema({ elements: parsed.elements, background: parsed.bgColor || parsed.background || "#FFFFFF", duration: parsed.duration || 5 });
        }
    ...
    <div style={{ width: "100vw", height: "100vh", overflow: "hidden" }}>
      <CanvasEditor
        ...
        onSave={async () => {
          setSaving(true);
          try {
            const key = templateId || `novo_${Date.now()}`;
            await supabase.from("system_config").upsert({
              key: `tmpl_${key}`,
              value: JSON.stringify({ ...schema, width: 1080, height: 1920 }),
