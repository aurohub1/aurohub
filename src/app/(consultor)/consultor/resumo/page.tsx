...
export default function ConsultorResumoPage() {
  ...
  async function loadData() {
    ...
    try {
      ...
      console.error("[ConsultorResumoPage] Erro:", err);
    } finally {
      setLoading(false);
    }
  }
  ...
  const metrics = useMemo(() => {
    ...
    void byFormat; void diffPercent;
    return { total, topDestino, avgPerWeek, mostActiveDayName, diff };
  ...
  const { startTour } = useTour({
    pageKey: "consultor-resumo",
    steps: [
      { element: "[data-tour='header-resumo']", popover: { title: "Resumo Mensal", description: "Veja um panorama das suas publicações. Use as setas para navegar entre meses ou exporte o relatório em PDF." } },
      ...
      { element: "[data-tour='chart-resumo']", popover: { title: "Gráfico de publicações", description: "Evolução das publicações por semana. Acompanhe os dias com mais e menos atividade." } },
    ],
    autoStart: true,
    delay: 1000,
  });
