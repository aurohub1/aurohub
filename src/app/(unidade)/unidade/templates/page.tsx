...
export default function UnidadeTemplatesPage() {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      console.error("[UnidadeTemplates] load:", err);
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  const useTemplate = (id: string) => {
    router.push(`/unidade/publicar?template=${id}`);
  };
        ...
        <div className="relative z-10 flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
          <div className="min-w-0 flex-1">
            ...
            <p className="mt-1 text-[12px] text-[var(--txt3)]">
              Modelos disponíveis para {profile?.store?.name || profile?.licensee?.name || "sua unidade"}.
            </p>
          </div>
        </div>
      </div>
