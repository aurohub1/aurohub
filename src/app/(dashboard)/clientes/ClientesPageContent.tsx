...
export default function ClientesPage() {
  ...
  const loadData = useCallback(async () => {
    try {
      ...
      // Mapeia licensee_id → maior percent de uso hoje
      const usageMap: Record<string, number> = {};
      for (const row of (usageR.data ?? []) as { licensee_id: string; count: number; limit_value: number }[]) {
        if (row.limit_value > 0) {
          const pct = Math.round((row.count / row.limit_value) * 100);
          usageMap[row.licensee_id] = Math.max(usageMap[row.licensee_id] ?? 0, pct);
        }
      }
                  ...
                  return (
                    <tr key={l.id} className="border-b border-[var(--bdr)] last:border-b-0 hover:bg-[var(--hover-bg)]">
                      ...
                      <td className="whitespace-nowrap px-4 py-3">
                        {(() => {
                          const pct = usageToday[l.id];
                          if (pct === undefined) return <span className="text-[11px] text-[var(--txt3)]">—</span>;
                          const color = pct >= 80 ? "var(--red, #EF4444)" : pct >= 60 ? "#F59E0B" : "var(--green, #22C55E)";
                          const bg    = pct >= 80 ? "rgba(239,68,68,0.1)" : pct >= 60 ? "rgba(245,158,11,0.1)" : "rgba(34,197,94,0.1)";
                          return (
                            <span className="inline-flex items-center gap-1 rounded-full px-2 py-0.5 text-[11px] font-semibold" style={{ color, background: bg }}>
                              {pct >= 80 && "⚠️ "}{pct}%
                            </span>
                          );
                        })()}
              ...
              {modalTab === "dados" && (
                <div className="flex flex-col gap-4">
                  ...
                  <div className="border-t border-[var(--bdr)] pt-4 mt-2">
                    ...
                    <div className="flex items-center justify-between rounded-lg border border-[var(--bdr)] px-3 py-2.5">
                      <div>
                        <div className="text-[12px] font-medium text-[var(--txt)]">Filtro de palavrão</div>
                        <div className="text-[10px] text-[var(--txt3)]">Bloqueia mensagens com conteúdo impróprio</div>
                      </div>
                      <button
                        type="button"
                        onClick={() => setForm(f => ({ ...f, chat_profanity_filter: !f.chat_profanity_filter }))}
                        className={`relative h-5 w-9 shrink-0 rounded-full transition-colors ${form.chat_profanity_filter ? "bg-emerald-500" : "bg-[var(--bdr2)]"}`}
                      >
                        <span className={`absolute top-0.5 h-4 w-4 rounded-full bg-white shadow transition-transform ${form.chat_profanity_filter ? "translate-x-4" : "translate-x-0.5"}`} />
                      </button>
                    </div>
                  </div>
                </div>
