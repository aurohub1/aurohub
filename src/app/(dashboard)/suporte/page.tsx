...
export default function SuporteADMPage() {
    ...
    <>
      <div className="flex items-end justify-between border-b border-[var(--bdr)] pb-4">
        <div>
          ...
          <p className="mt-0.5 text-[13px] text-[var(--txt3)]">
            {counts.open} aberto{counts.open !== 1 ? "s" : ""} · {counts.in_progress} em atendimento
          </p>
        </div>
      ...
      <div className="flex gap-1.5">
        {[{ k: "all", l: "Todos" }, ...Object.entries(STATUS).map(([k, v]) => ({ k, l: v.label }))].map(f => (
          <button key={f.k} onClick={() => setFilter(f.k)} className={`rounded-full px-3 py-1 text-[11px] font-semibold transition-colors ${filter === f.k ? "bg-[var(--blue)] text-white" : "bg-[var(--bg2)] text-[var(--txt3)]"}`}>
            {f.l}
          </button>
        ))}
      </div>
      ...
      {loading ? (
        ...
        <div className="flex flex-col items-center justify-center py-12 text-center">
          ...
          <p className="text-sm text-slate-400">Nenhum ticket.</p>
        </div>
      ) : (
          ...
          <div className="mx-4 flex max-h-[85vh] w-full max-w-lg flex-col overflow-hidden rounded-2xl border border-[var(--bdr)] bg-[var(--bg1)] shadow-2xl">
            ...
            {detail.status !== "closed" && (
              <div className="flex items-center gap-2 border-t border-[var(--bdr)] p-3">
                <input type="text" value={reply} onChange={e => setReply(e.target.value)} onKeyDown={e => e.key === "Enter" && sendReply()} placeholder="Responder..." className="flex-1 rounded-lg border border-[var(--bdr)] bg-transparent px-3 py-2 text-[13px] text-[var(--txt)] outline-none focus:border-[var(--blue)]" />
