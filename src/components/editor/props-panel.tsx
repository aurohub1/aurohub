...
function DesignTab({ s, u, allElements, onAlign, onOpenCrop, formType }: { s: EditorElement; u: (up: Partial<EditorElement>) => void; allElements: EditorElement[]; onAlign: (a: string) => void; onOpenCrop?: () => void; formType?: string }) {
  return (
    <>
      ...
      {/* Configuração do Formulário — só aparece quando bindParam está definido */}
      {s.bindParam && (
        <Sec t="Configuração do Formulário">
          <F l="Label do campo">
            <input
              type="text"
              value={s.formField?.label || ""}
              onChange={e => u({ formField: { ...(s.formField || { inputType: "text", label: "" }), label: e.target.value } })}
              placeholder={`Ex: ${s.bindParam.charAt(0).toUpperCase() + s.bindParam.slice(1)}`}
              style={inpS}
            />
          </F>
          <F l="Tipo de campo">
            <select
              value={s.formField?.inputType || "text"}
              onChange={e => u({ formField: { ...(s.formField || { label: s.bindParam || "", inputType: "text" }), inputType: e.target.value as any } })}
              style={selS}
            >
          ...
          <F l="Placeholder">
            <input
              type="text"
              value={s.formField?.placeholder || ""}
              onChange={e => u({ formField: { ...(s.formField || { inputType: "text", label: "" }), placeholder: e.target.value } })}
              placeholder="Texto de ajuda..."
              style={inpS}
            />
          </F>
          {s.formField?.inputType === "select" && (
            <F l="Opções (separadas por vírgula)">
              <textarea
                value={s.formField?.options?.join(", ") || ""}
                onChange={e => {
                  const opts = e.target.value.split(",").map(o => o.trim()).filter(Boolean);
                  u({ formField: { ...(s.formField || { inputType: "select", label: "" }), options: opts } });
                }}
                rows={3}
                placeholder="Ex: Opção 1, Opção 2, Opção 3"
                style={{ ...inpS, height: "auto", resize: "vertical", padding: "6px 8px" }}
              />
            </F>
          )}
          <div style={{ display: "flex", gap: 8, marginTop: 4 }}>
            <label style={{ display: "flex", alignItems: "center", gap: 4, fontSize: 10, cursor: "pointer", color: "var(--ed-txt2)" }}>
              <input
                type="checkbox"
                checked={s.formField?.required || false}
                onChange={e => u({ formField: { ...(s.formField || { inputType: "text", label: "" }), required: e.target.checked } })}
            ...
            <label style={{ display: "flex", alignItems: "center", gap: 4, fontSize: 10, cursor: "pointer", color: "var(--ed-txt2)" }}>
              <input
                type="checkbox"
                checked={s.formField?.uppercase || false}
                onChange={e => u({ formField: { ...(s.formField || { inputType: "text", label: "" }), uppercase: e.target.checked } })}
                style={{ accentColor: "var(--ed-accent)" }}
              />
              MAIÚSCULAS
            </label>
          ...
          {s.bindParam === "destino" && (
            <label style={{ display: "flex", alignItems: "center", gap: 4, fontSize: 10, cursor: "pointer", color: "var(--ed-txt2)", marginTop: 4 }}>
              <input
                type="checkbox"
                checked={s.formField?.triggerImgFundo || false}
                onChange={e => u({ formField: { ...(s.formField || { inputType: "text", label: "" }), triggerImgFundo: e.target.checked } })}
                style={{ accentColor: "var(--ed-accent)" }}
              />
              Buscar imagem de fundo automaticamente
            </label>
          )}
        </Sec>
      )}
