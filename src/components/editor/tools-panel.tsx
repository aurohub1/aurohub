...
export default function ToolsPanel(p: Props) {
  ...
  return (
    <div style={{ width: 50, display: "flex", flexDirection: "column", background: "var(--ed-surface)", borderRight: "1px solid var(--ed-bdr)", flexShrink: 0 }}>
      <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 1, padding: "6px 0", flex: 1, overflowY: "auto" }}>
        <GL>SEL</GL>
        <TB icon={<MousePointer2 size={18} />} t="Selecionar (V)" o={() => {}} />

        <GL>FORM</GL>
        <TB icon={<Square size={18} />} t="Retângulo (R)" o={() => add("rect")} />
        <TB icon={<Square size={18} strokeDasharray="4 2" />} t="Rect Arredondado" o={() => add("rect", { cornerRadius: 40, fill: "#1E3A6E" })} />
        <TB icon={<Circle size={18} />} t="Círculo (O)" o={() => add("circle")} />
        <TB icon={<Minus size={18} />} t="Linha (L)" o={() => add("rect", { name: "Linha", height: 4, cornerRadius: 2, fill: "#FFFFFF" })} />

        <GL>TEXT</GL>
        <TB icon={<Type size={18} />} t="Texto (T)" o={() => add("text")} />

        <GL>MÍDIA</GL>
        <TB icon={<ImageIcon size={18} />} t="Imagem (I)" o={() => {
        ...
        <GL>QR</GL>
        <TB icon={<QrCode size={18} />} t="QR Code" o={() => add("qrcode")} />

        <GL>BIND</GL>
        {/* Bind de Imagem — placeholder que no cliente vira a imagem real do form */}
        <div style={{ position: "relative" }} className="group">
          <TB icon={<ImageIcon size={18} strokeDasharray="4 2" />} t="Bind Imagem" o={() => add("imageBind")} gold />
        ...
        <div style={{ position: "relative" }} className="group">
          <TB icon={<Hexagon size={18} />} t="Campo Bind" o={() => {}} gold />
        ...
        <GL>OBJ</GL>
        <TB icon={<ArrowUp size={18} />} t="Camada +" o={() => p.onMoveLayer("up")} />
